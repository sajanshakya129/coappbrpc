...
class GreeterServiceImpl
    : public Greeter { // User defined class name GreeterServiceImpl which
                       // inherits from Greeter class.
public:
  GreeterServiceImpl(){};

  virtual void SayHello(RpcController *controller, const HelloRequest *request,
                        HelloReply *reply, Closure *done) {
    // Defining Function named "SayHello" which is defined as method in protobuf
    // file. This is where you do
    // processing and generate result and set result.
    std::string prefix("Hello");
    reply->set_message(prefix +
                       request->name()); // Setting  message and accessing
...
int main() {
  ...
  server.registerService(
      new ::coappbrpc::api::GreeterServiceImpl()); // Registering service
  // server.initiateServer(); //running server
  server.initiateServer(
      "localhost",
      "5683"); // You can run server by providing server_address and port no.
  return 0;
}
