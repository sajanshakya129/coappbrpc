...
// in command prompt. <filename.proto> will generate "filename.pb.h" and
// "filename.pb.cc"
#include <iostream>
#include <string>
...
using ::coappbrpc::api::HelloReply; // Must be included in code : User defined
                                    // reply in protofile
using ::coappbrpc::api::HelloRequest; // Must be included in code : User defined
                                      // request in protofile
using namespace std;

class GreeterClient { // Defining Client Class
public:
  string SayHello(
      const std::string &user) { // Defining method ping where we use RPC call

    HelloRequest request;
    request.set_name(user); // Setting user msg to request.
    // Note: In set_name, name is the parameter that we defined in protofile
    // request.
    HelloReply reply;
...
int main(void) {
  ...
  client->setServerAddr(
      "localhost",
      "5683"); // setting up Server address. Default is "localhost:5683".
